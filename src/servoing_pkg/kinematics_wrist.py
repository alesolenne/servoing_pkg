...
def get_jacobian(joint_angles):
    ...
    Parameters
    ----------
    joint_angles : [8x1] np.ndarray
                   Joint state vector
            
    Returns
    -------
    J : [6x8] np.ndarray
        Geometric Jacobian for Panda robot + wrist  
    """    
    dh_params = np.array([[0, 0.333, 0, joint_angles[0]],
                 ...
                 [0.105, 0, -np.pi / 2, joint_angles[7]]], dtype=np.float64)
...
def check_joints_position(q):
    ...
    # rospy.loginfo("Posizione di controllo: (%s, %s, %s, %s ,%s, %s, %s, %s)" , q[0][0], q[1][0], q[2][0], q[3][0], q[4][0], q[5][0], q[6][0], q[7][0])
    return q
...
def check_joints_velocity(q_dot):
    ...
    # rospy.loginfo("Velocita di controllo: (%s, %s, %s, %s ,%s, %s, %s, %s)" , q_dot[0][0], q_dot[1][0], q_dot[2][0], q_dot[3][0], q_dot[4][0], q_dot[5][0], q_dot[6][0], q_dot[7][0])
    return q_dot
