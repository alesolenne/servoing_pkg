...
def check_q_goal(q_goal):
    """Check if the robot have reached the desired position."""
    while True:
        joint_states = rospy.wait_for_message('/robot/joint_states', JointState)

        if joint_states.name == JOINT_NAMES_ARM:
            q_actual = np.array(joint_states.position).reshape((7, 1))
            e_q = q_goal - q_actual
...
def servoing():
    ...
    while not rospy.is_shutdown():
        try:
            # Get the transformation from the robot base to the tool extremity
            (t_0V, q_0V) = listener.lookupTransform('/robot_arm_link0', '/tool_extremity', rospy.Time(0))
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            # If the transformation is not available, wait and retry
            rospy.sleep(0.01)
            continue

        # Convert transformations to numpy arrays for computation
        t_0B, q_0B = np.array(t_0B), np.array(q_0B)
        t_0V, q_0V = np.array(t_0V), np.array(q_0V)

        # Compute joint velocities and error using the feedback control law
        q_dot, e = feedback(t_0B, q_0B, t_0V, q_0V, q)

        # Update joint positions based on computed velocities
        q = q + q_dot / CONTROL_FREQUENCY

        # Ensure joint positions and velocities are within valid limits
        q = check_joints_position(q)
        q_dot = check_joints_velocity(q_dot)

        # Create and populate the JointTrajectory message for the robot arm
        arm_str = JointTrajectory()
        arm_str.header = Header(stamp=rospy.Time.now())
        arm_str.joint_names = JOINT_NAMES_ARM
        arm_point = JointTrajectoryPoint(
            positions=q[:7, 0].tolist(),  # Arm joint positions
            velocities=q_dot[:7, 0].tolist(),  # Arm joint velocities
            accelerations=[0] * 7,  # No accelerations specified
            time_from_start=rospy.Duration(x)  # Time from start of trajectory
        )
        arm_str.points.append(arm_point)

        # Publish the trajectory commands to the respective topics
        pub_arm.publish(arm_str)

        # Check if the robot has reached the desired joint positions
        q = check_q_goal(q)

        # Compute the norms of the translation and orientation errors
        norm_e_t = np.linalg.norm(e[:3, :], 2)  # Translation error norm
        norm_e_o = np.linalg.norm(e[3:, :], 2)  # Orientation error norm

        # Log the error norms for debugging and monitoring
        rospy.loginfo(f"Translation error norm: {norm_e_t}")
        rospy.loginfo(f"Orientation error norm: {norm_e_o}")

        # Check if the errors are below the defined thresholds
        if norm_e_t < ERROR_TRANSLATION_THRESHOLD and norm_e_o < ERROR_ORIENTATION_THRESHOLD:
            rospy.loginfo("Visual servoing completed!")
            return True  # Exit the servoing loop

        # Increment the time step and sleep to maintain the control frequency
